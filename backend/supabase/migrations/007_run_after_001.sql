-- Run AFTER 001_core_schema.sql and 005_health_records_sync_ai.sql.
-- Applies sprint 2/3 tables and columns that depend on core schema.
-- Safe to re-run (IF NOT EXISTS / IF NOT EXISTS columns).

-- Sprint 2 & 3 tables (from 006, standalone so 006 need not be re-run)
...
-- user_challenge_daily_progress (from 004)
...
drop policy if exists "Users view own behavioral patterns" on public.behavioral_patterns;
create policy "Users view own behavioral patterns"
    on public.behavioral_patterns for select using (auth.uid() = user_id);

drop policy if exists "Foods are readable by authenticated users" on public.foods;
create policy "Foods are readable by authenticated users"
    on public.foods for select to authenticated using (true);

drop policy if exists "Users view own alert log" on public.alert_log;
create policy "Users view own alert log"
    on public.alert_log for select using (auth.uid() = user_id);

drop policy if exists "Challenge templates public read" on public.challenge_templates;
create policy "Challenge templates public read"
    on public.challenge_templates for select using (true);

drop policy if exists "Users can view own challenge daily progress" on public.user_challenge_daily_progress;
create policy "Users can view own challenge daily progress"
    on public.user_challenge_daily_progress for select using (auth.uid() = user_id);
